 ...
 * @author Russell Bent
 */
public class ScenarioVirtualFlowVariableFactory extends ScenarioVariableFactory<ElectricPowerNode, ElectricPowerModel> {

  /**
   * Constructor
   * @param scenarios
   */
  public ScenarioVirtualFlowVariableFactory(Collection<Scenario> scenarios) {
    super(scenarios);
  }
  ...
  public Collection<Variable> createVariables(MathematicalProgram program, ElectricPowerModel model) throws VariableExistsException, InvalidVariableException {
    ...
    for (ElectricPowerNode node : model.getNodes()) {
      for (Scenario scenario : getScenarios()) {
        variables.add(program.makeContinuousVariable(getVariableName(node, scenario)));        
      }
    }
