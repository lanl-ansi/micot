...
public class OpenDSSLoadFactory extends LoadFactory {
  ...
  public Load createLoad(ComObject iLoad, Bus bus, ComObject activeLoad)  {
    ...
    double reactiveLoad = iLoad.getDouble(OpenDSSIOConstants.LOAD_KVAR);
    double realLoad = iLoad.getDouble(OpenDSSIOConstants.LOAD_KW);                
  ...
  */
  ...
  private void fill(Load load, ComObject iLoad, Bus bus, ComObject activeLoad) {
    ...
    double reactiveLoad = iLoad.getDouble(OpenDSSIOConstants.LOAD_KVAR);
    double realLoad = iLoad.getDouble(OpenDSSIOConstants.LOAD_KW);         
    double actualReactiveLoad = iLoad.getDouble(OpenDSSIOConstants.LOAD_KVAR);
    double actualRealLoad = iLoad.getDouble(OpenDSSIOConstants.LOAD_KW);        
  ...
  public Load createLoad(String loadData, Bus bus) {
    ...
    double realLoad = Double.parseDouble(OpenDSSIOConstants.getData(loadData, "kW", "="));
    double reactiveLoad = Double.parseDouble(OpenDSSIOConstants.getData(loadData, "kvar", "="));
    Load load = registerLoad(legacyid);   
    initializeLoad(load, bus, realLoad, reactiveLoad);	
