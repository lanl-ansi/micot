...
public class OpenDSSLineFactory extends LineFactory {
  ...
  public Line createLine(ComObject objLine, Bus fromBus, Bus toBus, ComObject activeLine)  {
    ...
    double capacity = ampsRating * 14.4; // HACK!!! baseKV; TODO Fix this... need to figure out where to get the data
    ...
    //int numberOfPhases = objLine.getInteger(OpenDSSIOConstants.LINE_PHASES);
    ...
    line.setAttribute(Line.RESISTANCE_PHASE_A_KEY,rmatrix.get(0) * length);
    line.setAttribute(Line.RESISTANCE_PHASE_AB_KEY,rmatrix.get(1) * length);
    line.setAttribute(Line.RESISTANCE_PHASE_AC_KEY,rmatrix.get(2) * length);
    line.setAttribute(Line.RESISTANCE_PHASE_BA_KEY,rmatrix.get(3) * length);
    line.setAttribute(Line.RESISTANCE_PHASE_B_KEY,rmatrix.get(4) * length);
    line.setAttribute(Line.RESISTANCE_PHASE_BC_KEY,rmatrix.get(5) * length);
    line.setAttribute(Line.RESISTANCE_PHASE_CA_KEY,rmatrix.get(6) * length);
    line.setAttribute(Line.RESISTANCE_PHASE_CB_KEY,rmatrix.get(7) * length);
    line.setAttribute(Line.RESISTANCE_PHASE_C_KEY,rmatrix.get(8) * length);

    line.setAttribute(Line.REACTANCE_PHASE_A_KEY,xmatrix.get(0) * length);
    line.setAttribute(Line.REACTANCE_PHASE_AB_KEY,xmatrix.get(1) * length);
    line.setAttribute(Line.REACTANCE_PHASE_AC_KEY,xmatrix.get(2) * length);
    line.setAttribute(Line.REACTANCE_PHASE_BA_KEY,xmatrix.get(3) * length);
    line.setAttribute(Line.REACTANCE_PHASE_B_KEY,xmatrix.get(4) * length);
    line.setAttribute(Line.REACTANCE_PHASE_BC_KEY,xmatrix.get(5) * length);
    line.setAttribute(Line.REACTANCE_PHASE_CA_KEY,xmatrix.get(6) * length);
    line.setAttribute(Line.REACTANCE_PHASE_CB_KEY,xmatrix.get(7) * length);
    line.setAttribute(Line.REACTANCE_PHASE_C_KEY,xmatrix.get(8) * length);
  ...
  public Line createLine(String lineData, Bus fromBus, Bus toBus, String linecode) {
    ...
    double resistance = resistancePerLength * length;
    double reactance = reactancePerLength * length;    
