...
public class JSONResultExporter {
  ...
  public void exportJSON(OutputStream ps, ElectricPowerModel model, double totalTime, double simulationTime, SimulatorSolveState state) {
    ...
    for (Generator generator : model.getGenerators()) {
      ...
      generatorBuilder = generatorBuilder.add("mw_max", generator.getRealGenerationMax());
      generatorBuilder = generatorBuilder.add("mvar", generator.getReactiveGeneration().doubleValue());
      generatorBuilder = generatorBuilder.add("mvar_min", generator.getReactiveGenerationMin());
      generatorBuilder = generatorBuilder.add("mvar_max", generator.getReactiveGenerationMax());
    ...
    for (Load load : model.getLoads()) {
      ...
      loadBuilder = loadBuilder.add("mw", load.getRealLoad().doubleValue());
      loadBuilder = loadBuilder.add("mw_min", 0.0);
      loadBuilder = loadBuilder.add("mw_max", load.getRealLoadMax());
      loadBuilder = loadBuilder.add("mvar", load.getReactiveLoad().doubleValue());
      loadBuilder = loadBuilder.add("mvar_min", 0.0);
      loadBuilder = loadBuilder.add("mvar_max", load.getReactiveLoadMax());      
