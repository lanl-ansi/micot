 ...
 * @author Russell Bent
 */
public class MicrogridDiscreteObjectiveFunctionFactory implements ObjectiveFunctionFactory {
      ...
      for (Generator generator : node.getComponents(Generator.class)) {
        ...
        if (variable != null) {
          double cost = -generator.getAttribute(AlgorithmConstants.MICROGRID_FIXED_COST_KEY, Number.class).doubleValue() - 
              (generator.getAttribute(AlgorithmConstants.MICROGRID_COST_KEY, Number.class).doubleValue() *              
              generator.getAttribute(AlgorithmConstants.MAX_MICROGRID_KEY, Number.class).doubleValue());       
