...
/**
 * The amount of critical load needs to be at least a certain amount
 * 
 * @author Russell Bent
 */
public class ScenarioChanceConstrainedCriticalReactiveLoadMetConstraint extends ScenarioConstraintFactory<ElectricPowerNode, ElectricPowerModel> {
   ...
   * Constraint
   */
  public ScenarioChanceConstrainedCriticalReactiveLoadMetConstraint(double percentage, Collection<Scenario> scenarios) {
    super(scenarios);
  ...
  private String getConstraintName(Scenario scenario) {
    return "Crtical Chance Constraint Reactive Load Met-" + scenario;
  }
      ...
      for (ElectricPowerNode node : model.getNodes()) {
        for (Load load : node.getComponents(Load.class)) {
          ...
          if (variable != null) {
            ...
            totalLoad += load.getAttribute(Load.REACTIVE_LOAD_A_KEY, Number.class).doubleValue();
          }

          variable = factory.getVariable(problem, load, ScenarioReactiveLoadPhaseVariableFactory.PHASE_B, scenario);
          if (variable != null) {
            constraint.addVariable(variable, 1.0);
            totalLoad += load.getAttribute(Load.REACTIVE_LOAD_B_KEY, Number.class).doubleValue();
          }

          variable = factory.getVariable(problem, load, ScenarioReactiveLoadPhaseVariableFactory.PHASE_C, scenario);
          if (variable != null) {
            constraint.addVariable(variable, 1.0);
            totalLoad += load.getAttribute(Load.REACTIVE_LOAD_C_KEY, Number.class).doubleValue();
          }
        }
      }
      totalLoad /= mvaBase;
